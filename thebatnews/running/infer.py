...
from thebatnews.processing import fabrica
from typing import Generator, List, Union
from functools import partial

from thebatnews.processing.loader import NamedDataLoader
from thebatnews.processing.prime import IProcessor, ICLSFastProcessor

from thebatnews.etc.processor import grouper

from thebatnews.modeling.core import IRunner, M1Runner

from thebatnews.etc import initialize_device_settings, log_ascii_workers, calc_chunksize
from thebatnews.etc.prime import set_all_seeds
...
class INFRunner:
    ...
    .. code-block:: python
       ...
       model = INFRunner.load(your_model_dir)
    ...
    def __init__(
        ...
        :param model: M1Runner / M2Runner / M3Runner etc... to run in inference mode
        :type model: IRunner
        ...
        devices, n_gpu = initialize_device_settings(use_cuda=gpu, local_rank=-1)
    ...
    def load(
        ...
        **kwargs,
        ...
        if os.path.exists(model_name_or_path):
            runner = IRunner.load(load_dir=model_name_or_path, device=devices[0], strict=strict)
            # if task_type == "embeddings":
            #     processor = InferenceProcessor.load_from_dir(model_name_or_path)
            if task_type == "text_classification":
                processor = ICLSFastProcessor.load_from_dir(model_name_or_path)
            else:
                processor = IProcessor.load_from_dir(model_name_or_path)
            if runner.language_model.__class__.__name__ == "IE5Model":
                processor.process_fn = partial(fabrica.running_ie5, prefix="query: ")
            else:
                processor.process_fn = fabrica.running

        # b) or from remote transformers model hub
        else:
            raise Exception(f"Model not found at {str(model_name_or_path)}. Have you saved it properly")
        ...
        if self.process_pool is None:  # multiprocessing disabled (helpful for debugging or using in web frameworks)
            ...
            if streaming:
                if multiprocessing_chunksize is None:
                    logger.warning(
                        "Streaming mode is enabled for the Inferencer but multiprocessing_chunksize is not "
    ...
    def _get_predictions(self, dataset, tensor_names, baskets):
        ...
        for i, batch in enumerate(tqdm(data_loader, desc=f"Inferencing Samples", unit=" Batches", disable=self.disable_tqdm)):
            ...
            batch_samples = samples[i * self.batch_size : (+1) * self.batch_size]
