...
# from thebatnews.processing.module import pipe as ppipe
# from thebatnews.tooling import Email, stl
# from thebatnews.viewing.module import IFormat
...
#     2. As well as
#     """
#     response = [{"text": d.meta["raw"], "score": 0.5,
#                  "timestamp": d.meta["timestamp"]} for d in docs]
#     kw = KeywordProcessor()
...
#     # PRE-fix- VIEW
#     preview = {k: i + 1 for i, k in enumerate(kw.get_all_keywords().values())}
...
#                 posix.append({"lo": lo + prefcount, "hi": hi +
#                              prefcount, "color": preview[w]})
#                 if lo > 0:
...
#     right_date = dt.datetime.max if right_date is None else dp.parse(
#         right_date)
#     session["db"]
#     for k, _ in keywords:
...
#             daily_volume = ppipe.pipe_bound(
#                 connection, "redate", start_date, end_date).shape[0]
#             relative = int(
#                 np.round((((hi - lo) * 1.0 / daily_volume)) * 100.0))
#             qij[k].append({"timestamp": timestamp.strftime(
#                 "%d/%m/%y %H:%M:%S"), "value": hi - lo, "relative": relative})
#             lo = hi
#     return qij
...
#                             stop_words=list(stopwords)).fit(documents)
#     t = count.transform(documents).toarray()  # (num_topics, different_tokens)
#     # (num_topics,). How many tokens occur at specific `topic`
#     w = t.sum(axis=1)
#     # (num_tokens, num_topics). This step transforms `count` to `frequency`
#     tf = np.divide(t.T, w)
#     sum_t = t.sum(
#         axis=0
...
#     name, count = [" | ".join(str(d).split("_")[1:])
#                    for d in info["Name"]], list(info["Count"])
...
#         # Tricky way to add (+1) if (-1) is present and (+0) otherwise
#         topic_pos = int(topic_idx) + offset * (-1)
