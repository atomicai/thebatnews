...
# from thebatnews.processing import pipe as ppipe
from thebatnews.tdk import pipe
from thebatnews.tooling import initialize_device_settings
# from thebatnews.tooling import get_data, initialize_device_settings
# from thebatnews.viewing.module import plotly_wordcloud
...
devices, n_gpu = initialize_device_settings(
    use_cuda=torch.cuda.is_available(), multi_gpu=False)
...
model = SentenceTransformer(
    "paraphrase-multilingual-mpnet-base-v2", device=devices[0])
...
def search():
    ...
    #     return jsonify({"docs": []})
    # else:
    #     # response = pipe.pipe_paint_docs(
    #     #     docs=response, querix=querix, prefix=prefixer)
    #     response = {}
...
#     df = ppipe.pipe_silo(df, tecol, syms=[":"], wordlist=set(
#         stopper), date_col_name=dacol)
#     print("silo")
#     df = (
#         df.with_row_count()
...
#     _botpic = botpic.fire(min_topic_size=_min_topic_size, hdbscan_model=KMeans(
#         n_clusters=session["num_clusters"]))
#     topics, probs = _botpic.fit_transform(
#         docs, embeddings=embeddings)  # we use model under the hood
...
#     )
#     top_n_words = pipe.extract_top_n_words_per_topic(
...
#     engine.index([{"content": d, "timestamp": t, "raw": r, "topic_id": c}
#                  for d, t, r, c in zip(docs, times, raws, topics)])
#     store[uid] = engine
#     _plopics = (
...
#     report_filepath = Path(os.getcwd()) / ".cache" / \
#         uid / str(filename.stem + ".xlsx")
...
#     session["tropics"] = _botpic.visualize_barchart(
#         width=312.5, height=225, title="Topic Word Scores")
...
#                 },
#                 {"api": "viewing_timeseries_tropics",
#                     "title": "Keyword ranking per topics"},
#             ],
#         },
#         {
...
#         response = engine.retrieve_top_k(
#             querix, topic_ids=[q_idx], left_date=left_date, right_date=right_date, top_k=250)
#     except:
#         return jsonify({"docs": []})

#     docs = pipe.pipe_paint_docs(
#         docs=response, querix=querix, prefix=list(prefixer))
...
#         df = session["db"]
#         sub = [str(d) for d in list(
#             df.filter(pl.col("topic") == q_idx).select("silo"))[0]]
#         kods = pipe.pipe_paint_kods(
#             docs=sub, engine=engine, keyworder=iworder, left_date=left_date, right_date=right_date)
...
#     pipe.report_overall_snapshot(
#         query=session["query"], docs=data["docs"], filepath=filepath)
